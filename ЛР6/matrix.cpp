...
void Matrix::saveToBinary(const string& filename) const {
    ...
    ofs << *this;
    if (ofs.fail()) {
        throw ios_base::failure("Error during binray file writing");
    }
    ofs.close();
}
...
// Метод clone() для базового класса Matrix
Matrix* Matrix::clone() const {
    // Создаем новый объект Matrix, используя конструктор копирования
    return new Matrix(*this);
}

// Метод clone() для класса IdentityMatrix
IdentityMatrix* IdentityMatrix::clone() const {
    // Создаем новый объект IdentityMatrix, используя конструктор копирования
    return new IdentityMatrix(*this);
}

// Метод clone() для класса RectangularMatrix
RectangularMatrix* RectangularMatrix::clone() const {
    // Создаем новый объект RectangularMatrix, используя конструктор копирования
    return new RectangularMatrix(*this);
}


void List::append(List& list, Matrix* matrix) {
    Node* newNode = new Node();
    newNode->data = matrix->clone();
